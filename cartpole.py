...
def get_kinematics(env, device):
    return torch.Tensor([env.state]).to(device)
...
def main():
    ...
    # Not using images
    # plt.figure()
    # plt.imshow(get_screen(env, resize, device).cpu().squeeze(0).permute(1, 2, 0).numpy(),
    #            interpolation='none')
    ...
    init_kinematics = get_kinematics(env, device)
    kinematics_shape = init_kinematics.shape[1]
    # print(kinematics_shape)
    ...
    memory = ReplayMemory(50000, Transition)
    ...
    for i_episode in range(num_episodes):
        ...
        env.reset()
        state = get_kinematics(env, device)
        for t in count():
            # Select and perform an action
            action, steps_done = select_action(
                state, steps_done, n_actions, policy_net, EPS_START, EPS_END, EPS_DECAY, device)
            ...
            optimize_model(memory, policy_net, target_net,
                           optimizer, BATCH_SIZE, GAMMA, Transition, device)
            # Hard cap
            if t > 500:
                done = True
            if done:
        ...
        if i_episode % 100 == 0:
            print('Episode {}: {}'.format(i_episode, t+1))
    ...
    plt.savefig('cartpoleQlearning.png')
